...
4. **Scanned-document reader (OCR)** — saves the Gemini API key that `uv run startup.py --ocr` uses into the repo-root `.env`. You create the key at [Google AI Studio](https://aistudio.google.com/apikey) and paste it into the screen; nothing is installed or downloaded. OCR runs in Google's cloud with `gemini-3.8-flash`: every page image of a scanned PDF is uploaded, and Google bills your account per page (about a third of a cent per page at launch pricing). OCR never runs unless you ask for it, and the run prints its token and dollar total when it finishes.
...
4. **Scanned-document reader (OCR)** — paste your Gemini API key; it is saved to the repo-root `.env`. Nothing is installed.
...
> **About OCR:** scanned PDFs have no selectable text, so they need OCR before the AI can read them. OCR uploads each page image to Google (Gemini) and costs money, so it does **not** run automatically. If `startup.py` reports PDFs that need it, ask before running `uv run startup.py --ocr`. OCR also needs a Gemini API key first — run `uv run config.py` yourself and pick **Scanned-document reader (OCR)**, then paste the key you created at https://aistudio.google.com/apikey.
