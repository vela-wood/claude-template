...
import startup
import startup_lib.certify
import startup_lib.common
import startup_lib.convert
import startup_lib.discovery
import startup_lib.indexes
import startup_lib.migration
import startup_lib.ocr
...
def test_discovery_prunes_hidden_dirs_and_caption_cache(repo_tmp):
    ...
    sources, mbx = startup_lib.discovery.discover_sources(repo_tmp)
...
def test_second_run_performs_zero_conversions(repo_tmp, monkeypatch, capsys):
    ...
    calls = []
    original = startup_lib.convert.convert_to_markdown
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", lambda s: calls.append(s) or original(s)
    )
    assert run_main() == 0
    out = capsys.readouterr().out
    assert calls == []
...
def test_missing_token_row_is_retokenized_not_reconverted(
    ...
    calls = []
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", lambda s: calls.append(s) or "x")
    assert run_main() == 0
    out = capsys.readouterr().out
...
def test_new_source_hash_failure(repo_tmp, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        startup_lib.common, "hash_file", _raise_for(bad, startup_lib.common.hash_file))
...
def test_indexed_source_hash_failure_preserves_prior_state(
    ...
    calls = []
    original_convert = startup_lib.convert.convert_to_markdown
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown",
...
def test_failed_conversion_keeps_stale_sidecar_and_old_hash(
    ...
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", boom)
    assert run_main() == 1
    out = capsys.readouterr().out
    ...
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", original)
    assert run_main() == 0
...
def test_token_count_failure_preserves_prior_certification(
    ...
    monkeypatch.setattr(
        startup_lib.common, "count_tokens", boom)
    assert run_main() == 1
    assert (repo_tmp / "a.eml.md").read_bytes() == old_sidecar
...
def test_source_mutation_during_conversion_fails(repo_tmp, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", mutate_then_convert)
    assert run_main() == 1
    out = capsys.readouterr().out
...
def test_empty_converter_output_is_failure(repo_tmp, monkeypatch, capsys):
    write_eml(repo_tmp / "a.eml")
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", lambda s: "")
    assert run_main() == 1
    assert not (repo_tmp / "a.eml.md").exists()
...
def test_converter_concurrency_capped_at_four(repo_tmp, monkeypatch):
    ...
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", tracking_convert)
    assert run_main() == 0
...
def _authorize_ocr(monkeypatch, fake=None):
    """`--ocr` with a key present and the Gemini client replaced by a fake.
    Returns the fake so tests can inspect calls."""
    import sys

    fake = fake or FakePageOcr()
    monkeypatch.setattr(sys, "argv", ["startup.py", "--ocr"])
    monkeypatch.setenv("GEMINI_API_KEY", "test-key")
    monkeypatch.setattr(startup_lib.ocr, "_make_client", lambda key, level: fake)
    return fake
...
def test_pending_ocr_without_flag_is_deferred_exit_zero(repo_tmp, monkeypatch, capsys):
    ...
    assert "uploads page images to Google" in out  # consent covers the upload
...
def test_unresolved_scanned_pdf_conflict_is_excluded_from_every_route(
    ...
    monkeypatch.setattr(
        startup_lib.convert, "classify_pdf", should_not_run)
    monkeypatch.setattr(startup, "run_ocr", should_not_run)
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", should_not_run)
    monkeypatch.setattr(
        startup_lib.common, "count_tokens", should_not_run)
...
def test_requested_ocr_success_certifies_exactly_once(repo_tmp, monkeypatch, capsys):
    ...
    fake = _authorize_ocr(monkeypatch)
    calls = []
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", lambda s: calls.append(s) or "x"
    )
    ...
    out = capsys.readouterr().out
    assert "1 converted, 0 unchanged, 0 failed, 0 deferred for OCR" in out
    assert "uploaded to Google" in out
    assert calls == []  # OCR-routed source never invokes the generic converter
    assert fake.calls == 2
    sidecar = (repo_tmp / "scan.pdf.md").read_text(encoding="utf-8")
    assert sidecar.startswith("<!-- OCR by gemini-3.8-flash on ")
    assert sidecar.count("Text of call") == 2  # both pages, joined in order
    assert sidecar.index("<!-- page 1 -->") < sidecar.index("<!-- page 2 -->")
    ocr_rows = read_csv_dict(repo_tmp / ".ocr_index.csv")
...
def test_requested_ocr_failure_is_nonzero_and_never_falls_through(
    ...
    make_scanned_pdf(repo_tmp / "scan.pdf")
    _authorize_ocr(monkeypatch, FakePageOcr(fail_call=1))
    calls = []
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", lambda s: calls.append(s) or "x"
    )
    ...
    out = capsys.readouterr().out
    assert "0 converted, 0 unchanged, 1 failed" in out
    assert "page(s) failed: p1: boom" in out
    assert calls == []
    assert not (repo_tmp / "scan.pdf.md").exists()
...
def test_missing_key_fails_pending_and_never_uploads(repo_tmp, monkeypatch, capsys):
    import sys
    ...
    monkeypatch.setattr(
        startup_lib.ocr, "_make_client",
        lambda key, level: (_ for _ in ()).throw(AssertionError("client built without a key")),
    )

    assert run_main() == 1
    out = capsys.readouterr().out
    assert "GEMINI_API_KEY is not set" in out
    assert "uv run config.py" in out
    assert "0 converted, 0 unchanged, 1 failed" in out
    assert not (repo_tmp / "scan.pdf.md").exists()
    assert read_csv_dict(repo_tmp / ".ocr_index.csv")[0]["ocr_done"] == ""
    assert read_csv_dict(repo_tmp / ".hash_index.csv") == []
...
# ---------------------------------------------------------------------------
# OCR partial success and interruption
# ---------------------------------------------------------------------------


def _two_scans(repo_tmp):
    """Two 2-page scanned PDFs, submitted in name order (one, two)."""
    make_scanned_pdf(repo_tmp / "one.pdf", pages=2)
    make_scanned_pdf(repo_tmp / "two.pdf", pages=2)


def test_page_failure_keeps_completed_pdfs_and_fails_only_the_rest(
    repo_tmp, monkeypatch, capsys
):
    """A failed page fails its own PDF; the PDFs that finished are kept."""
    _two_scans(repo_tmp)
    # pages are requested in submission order: one p1, one p2, two p1, ...
    _authorize_ocr(monkeypatch, FakePageOcr(fail_call=3))

    assert run_main() == 1
    out = capsys.readouterr().out
    assert "1 converted, 0 unchanged, 1 failed" in out
    assert "ERROR OCRing two.pdf" in out
...
def test_interrupt_keeps_completed_pdfs_and_fails_the_rest(
    ...
    _two_scans(repo_tmp)
    _authorize_ocr(monkeypatch)

    async def finish_one_then_interrupt(root, rels, hashes, client, concurrency, run, defer):
        rel = rels[0]
        run.results.append(
            _finalize_sidecar(root, root / rel, hashes[rel], "one page", "gemini-ocr", ocr_done=True)
        )
        raise KeyboardInterrupt

    monkeypatch.setattr(startup_lib.ocr, "ocr_pdfs", finish_one_then_interrupt)
...
def test_run_ocr_passes_deferred_commits_to_the_driver(repo_tmp, monkeypatch):
    """Migration may ask for a sidecar commit to be deferred; that set must
    reach the driver, which threads it into _finalize_sidecar."""
    make_scanned_pdf(repo_tmp / "scan.pdf")
    monkeypatch.setenv("GEMINI_API_KEY", "test-key")
    monkeypatch.setattr(startup_lib.ocr, "_make_client", lambda key, level: FakePageOcr())
    seen = {}

    async def record(root, rels, hashes, client, concurrency, run, defer):
        seen["defer"] = defer

    monkeypatch.setattr(startup_lib.ocr, "ocr_pdfs", record)
    startup_lib.ocr.run_ocr(repo_tmp, ["scan.pdf"], {"scan.pdf": "h"}, {"scan.pdf"})
    assert seen["defer"] == {"scan.pdf"}
...
def test_index_write_failure_exits_nonzero(repo_tmp, monkeypatch, capsys):
    ...
    original = startup_lib.certify.atomic_write_text

    def targeted_write(path, text, newline=""):
        if path.name == startup_lib.common.HASH_INDEX_FILENAME:
            raise OSError("injected index write failure")
        return original(path, text, newline=newline)

    monkeypatch.setattr(
        startup_lib.certify, "atomic_write_text", targeted_write)
    assert run_main() == 1
...
def test_token_write_failure_withholds_certification_marker(repo_tmp, monkeypatch):
    ...
    original_ocr_serializer = startup_lib.certify.serialize_ocr_index

    def targeted_write(path, text, newline=""):
        writes.append(path.name)
        if path.name == startup_lib.common.TOKEN_INDEX_FILENAME:
    ...
    monkeypatch.setattr(
        startup_lib.certify, "atomic_write_text", targeted_write)
    monkeypatch.setattr(
        startup_lib.certify, "serialize_hash_index", hash_should_not_serialize)
    monkeypatch.setattr(startup_lib.certify, "serialize_ocr_index", count_ocr_serialization)
    errors = startup_lib.certify.persist_indexes(
        repo_tmp,
        {"a.eml": "0badf00d"},
        {"a.eml.md": 999},
        {
    ...
    assert writes == [startup_lib.common.TOKEN_INDEX_FILENAME, startup.OCR_INDEX_FILENAME]
    assert ocr_serializations == 1
    assert not list(repo_tmp.glob(".*.tmp"))


@pytest.mark.parametrize(
    "failed_name", [startup_lib.common.TOKEN_INDEX_FILENAME, startup.OCR_INDEX_FILENAME]
)
...
def test_ocr_write_failure_withholds_hash_without_serializing_it(
    ...
    write_eml(repo_tmp / "a.eml")
    assert run_main() == 0
    prior_hash = (repo_tmp / startup_lib.common.HASH_INDEX_FILENAME).read_bytes()
    current_tokens = token_map(repo_tmp)
    original_write = startup_lib.certify.atomic_write_text
    writes = []
    ...
    monkeypatch.setattr(
        startup_lib.certify, "atomic_write_text", fail_ocr_write)
    monkeypatch.setattr(
        startup_lib.certify, "serialize_hash_index", hash_should_not_serialize)
    errors = startup_lib.certify.persist_indexes(
        repo_tmp,
    ...
    assert (repo_tmp / startup_lib.common.HASH_INDEX_FILENAME).read_bytes() == prior_hash
...
def test_persist_indexes_serializes_each_index_exactly_once(
    ...
    writes = []
    original_token = startup_lib.indexes.serialize_token_index
    original_ocr = startup_lib.certify.serialize_ocr_index
    original_hash = startup_lib.indexes.serialize_hash_index
    original_write = startup_lib.certify.atomic_write_text
    ...
    monkeypatch.setattr(
        startup_lib.certify, "serialize_token_index", count_token)
    monkeypatch.setattr(startup_lib.certify, "serialize_ocr_index", count_ocr)
    monkeypatch.setattr(
        startup_lib.certify, "serialize_hash_index", count_hash)
    monkeypatch.setattr(
        startup_lib.certify, "atomic_write_text", record_write)

    assert startup_lib.certify.persist_indexes(
        repo_tmp, {"a.eml": "deadbeef"}, {"a.eml.md": 3}, {}
    ) == []
    assert calls == {"token": 1, "ocr": 1, "hash": 1}
    assert writes == [
        startup_lib.common.TOKEN_INDEX_FILENAME,
        startup.OCR_INDEX_FILENAME,
        startup_lib.common.HASH_INDEX_FILENAME,
    ]


def test_shared_index_serializer_preserves_headers_sorting_and_newlines():
    assert startup_lib.indexes._serialize_index({"b": 2, "a": 1}, "tokens") == (
        "file,tokens\r\na,1\r\nb,2\r\n"
    )
    assert startup_lib.indexes.serialize_hash_index({"b": "bb", "a": "aa"}) == (
        "file,hash\r\na,aa\r\nb,bb\r\n"
    )
...
def test_sidecar_naming_for_sigcheck(repo_tmp, monkeypatch):
    ...
    for name in ("x.pdf", "x.docx", "x.eml", "x.msg", "x.mbox", "x.mht"):
        monkeypatch.setattr(
        startup_lib.common, "SIDECAR_DOTFILES", False)
        assert startup_lib.common.converted_path(repo_tmp / name).name == f"{name}.md"
        assert startup_lib.common.other_style_path(repo_tmp / name).name == f".{name}.md"
        monkeypatch.setattr(
        startup_lib.common, "SIDECAR_DOTFILES", True)
        assert startup_lib.common.converted_path(repo_tmp / name).name == f".{name}.md"
        assert startup_lib.common.other_style_path(repo_tmp / name).name == f"{name}.md"
    monkeypatch.setattr(
        startup_lib.common, "SIDECAR_DOTFILES", False)
    with pytest.raises(ValueError):
        startup_lib.common.converted_path(repo_tmp / "x.mbx")
    with pytest.raises(ValueError):
        startup_lib.common.other_style_path(repo_tmp / "x.mbx")


def test_sidecar_helper_and_wrappers_read_setting_at_call_time(repo_tmp, monkeypatch):
    source = repo_tmp / "x.docx"
    assert startup_lib.common._sidecar_path(source, dotted=False).name == "x.docx.md"
    assert startup_lib.common._sidecar_path(source, dotted=True).name == ".x.docx.md"

    monkeypatch.setattr(
        startup_lib.common, "SIDECAR_DOTFILES", False)
    assert startup_lib.common.converted_path(source).name == "x.docx.md"
    monkeypatch.setattr(
        startup_lib.common, "SIDECAR_DOTFILES", True)
    assert startup_lib.common.converted_path(source).name == ".x.docx.md"
...
def write_token_map(repo_tmp, rows: dict[str, int]) -> None:
    (repo_tmp / ".token_index.csv").write_text(
        startup_lib.indexes.serialize_token_index(rows), encoding="utf-8"
    )
...
def test_flip_preference_renames_and_stays_certified(repo_tmp, monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", lambda s: calls.append(s) or "x")
    monkeypatch.setattr(
        startup_lib.common, "count_tokens",
...
def test_inverse_regeneration_token_write_failure_cannot_stale_certify(
    ...
    monkeypatch.setattr(
        startup_lib.certify, "atomic_write_text", fail_token_write)
    assert run_main() == 1
    ...
    monkeypatch.setattr(
        startup_lib.certify, "atomic_write_text", original_write)
    conversions = []
    original_convert = startup_lib.convert.convert_to_markdown
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown",
...
def test_single_unindexed_sidecar_preserved_before_conversion(
    ...
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown", convert)
    assert run_main() == 0
...
def test_both_identical_no_rows_retokenizes_matching_hash(repo_tmp, monkeypatch):
    ...
    (repo_tmp / ".token_index.csv").write_text("file,tokens\n", encoding="utf-8")
    calls = []
    original_count = startup_lib.common.count_tokens
    monkeypatch.setattr(
        startup_lib.common, "count_tokens",
...
def test_two_phase_preservation_failure_leaves_both_candidates(
    ...
    original_copy = startup_lib.migration._copy_to_unique_backup
    calls = 0
    ...
    monkeypatch.setattr(
        startup_lib.migration, "_copy_to_unique_backup", fail_second)
    monkeypatch.setattr(
        startup_lib.convert, "convert_to_markdown",
        lambda source: (_ for _ in ()).throw(AssertionError("conflict excluded")),
    )
...
def test_conflict_backup_retries_exclusive_name_without_clobber(
    ...
    monkeypatch.setattr(
        startup_lib.migration.uuid, "uuid4", lambda: FakeUUID(next(values))
    )
    backup = startup_lib.migration._copy_to_unique_backup(candidate)
...
def test_unhashable_preference_flip_retains_then_recovers(
    ...
    stored_count = token_map(repo_tmp)["a.eml.md"]
    set_dotfiles(repo_tmp, True)
    original_hash = startup_lib.common.hash_file
    monkeypatch.setattr(
        startup_lib.common, "hash_file", _raise_for(source, original_hash))
    ...
    monkeypatch.setattr(
        startup_lib.common, "hash_file", original_hash)
    monkeypatch.setattr(
        startup_lib.common, "count_tokens",
        lambda path: (_ for _ in ()).throw(AssertionError("must trust stored count")),
    )
...
def test_canonical_rename_failure_is_unresolved_and_preserves_authority(
    ...
    stored_count = token_map(repo_tmp)["a.eml.md"]
    set_dotfiles(repo_tmp, True)
    original_replace = startup_lib.migration.os.replace
    ...
    monkeypatch.setattr(startup_lib.migration.os, "replace", targeted_replace)
    assert run_main() == 1
...
def test_unhashable_both_file_conflict_is_reported_without_mutation(
    ...
    write_token_map(repo_tmp, {"a.eml.md": 1, ".a.eml.md": 2})
    monkeypatch.setattr(
        startup_lib.common, "hash_file", _raise_for(source, startup_lib.common.hash_file))
